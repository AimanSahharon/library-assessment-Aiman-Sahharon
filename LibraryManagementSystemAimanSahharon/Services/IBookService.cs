...
namespace LibraryManagementSystemAimanSahharon.Services
{
        ...
        {
            Task<IEnumerable<Book>> GetAllBooksAsync(string? authorFilter, string? titleFilter, string? sortBy, string? isbnFilter);
            Task<Book?> GetBookByIdAsync(int id);
            Task<Book> CreateBookAsync(Book book);
            Task<Book?> UpdateBookAsync(int id, Book updated);
            Task<bool> DeleteBookAsync(int id);
            Task<bool> IsIsbnExistsAsync(string isbn); //to ensure no 2 different books have the same isbn number
